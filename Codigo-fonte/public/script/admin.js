...
async function carregarDashboard() {
    ...
    // Se já existe um gráfico, destrói para não sobrepor
    if (meuGrafico) meuGrafico.destroy();

    // Cálculos para o gráfico
    const total = dados.totalItens || 1; // Evita divisão por zero
    const devolvidos = dados.totalItens - dados.itensReivindicados; // Aproximação baseada nos seus dados
    const reivindicados = dados.itensReivindicados;
...
function exportarRelatorioCSV() {
    ...
    // Criação do link de download oculto
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    
    link.setAttribute("href", url);
    link.setAttribute("download", "relatorio_achados_perdidos_icet.csv");
